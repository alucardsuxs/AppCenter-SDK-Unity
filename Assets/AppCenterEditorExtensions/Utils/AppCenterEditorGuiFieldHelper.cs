...
namespace AppCenterEditor
{
    public static class AppCenterGuiFieldHelper
    {
        ...
        public static void SuperFancyDropdown<T>(float labelWidth, string label, T activeElement, IList<T> elements, Func<T, string> getElementKey, Action<T> OnChangeTo, GUIStyle style, params GUILayoutOption[] options)
        {
            ...
            {
                EditorGUILayout.LabelField(label, AppCenterEditorHelper.uiStyle.GetStyle("labelStyle"), GUILayout.Width(labelWidth));
                var prevIndex = IndexOf(namesList, getElementKey(activeElement));
                var newIndex = EditorGUILayout.Popup(prevIndex, namesList, AppCenterEditorHelper.uiStyle.GetStyle("TextField"), GUILayout.MinHeight(25));
    ...
    public class FixedWidthLabel : IDisposable
    {
        ...
        public FixedWidthLabel(GUIContent label, GUIStyle style) // constructor.
        {
            ...
            EditorGUILayout.BeginHorizontal(AppCenterEditorHelper.uiStyle.GetStyle("gpStyleClear")); // create a new horizontal group
        ...
        public FixedWidthLabel(string label)
            : this(new GUIContent(label), AppCenterEditorHelper.uiStyle.GetStyle("labelStyle")) //alternative constructor, if we don't want to deal with GUIContents
        {
        }
